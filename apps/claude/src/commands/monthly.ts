...
export const monthlyCommand = define({
	...
	async run(ctx) {
		...
		if (useJson) {
			...
			// Print header
			logger.box('LLM Water Tracker - Monthly');
			...
			// Add totals
			const totalWaterMl = monthlyData.reduce((sum, data) => sum + calculateWaterMlForEntry(
				{ inputTokens: data.inputTokens, outputTokens: data.outputTokens, cacheCreationTokens: data.cacheCreationTokens, cacheReadTokens: data.cacheReadTokens },
				data.modelsUsed ?? [],
				waterRates,
			), 0);
			const totalCo2Grams = monthlyData.reduce((sum, data) => sum + calculateCo2GramsForEntry(
				{ inputTokens: data.inputTokens, outputTokens: data.outputTokens, cacheCreationTokens: data.cacheCreationTokens, cacheReadTokens: data.cacheReadTokens },
				data.modelsUsed ?? [],
				co2Rates,
			), 0);
