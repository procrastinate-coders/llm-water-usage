...
- Treat Amp as a sibling to `apps/claude`, `apps/codex`, and `apps/opencode`.
- Reuse shared packages (`@llm-water-tracker/terminal`, `@llm-water-tracker/internal`) wherever possible.
...
- `llm-water-amp daily` - Show daily usage report
- `llm-water-amp monthly` - Show monthly usage report
- `llm-water-amp session` - Show usage by thread (session)
