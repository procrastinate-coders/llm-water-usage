...
export const sessionCommand = define({
	...
	async run(ctx) {
		...
		const totals = {
			...
			waterLiters: sessionData.reduce((sum, s) => sum + s.waterLiters, 0),
			waterGallons: sessionData.reduce((sum, s) => sum + s.waterGallons, 0),
		};
		...
		for (const parentSession of parentSessions) {
			...
			table.push([
				...
				formatWaterLiters(parentSession.waterLiters),
				formatWaterGallons(parentSession.waterGallons),
			]);
			...
			if (subSessions != null && subSessions.length > 0) {
				for (const subSession of subSessions) {
					table.push([
						...
						formatWaterLiters(subSession.waterLiters),
						formatWaterGallons(subSession.waterGallons),
					]);
				}
				...
				const subtotalWaterLiters =
					parentSession.waterLiters + subSessions.reduce((sum, s) => sum + s.waterLiters, 0);
				const subtotalWaterGallons =
					parentSession.waterGallons + subSessions.reduce((sum, s) => sum + s.waterGallons, 0);
				...
				table.push([
					...
					pc.yellow(formatWaterLiters(subtotalWaterLiters)),
					pc.yellow(formatWaterGallons(subtotalWaterGallons)),
				]);
			}
		}
