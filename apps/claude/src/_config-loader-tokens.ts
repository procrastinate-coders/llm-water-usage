 ...
 * 1. Local .llm-water-tracker/llm-water-tracker.json
 * 2. User config directories from getClaudePaths() + llm-water-tracker.json
 */
function getConfigSearchPaths(): string[] {
	const claudeConfigDirs = [join(process.cwd(), '.llm-water-tracker'), ...toArray(getClaudePaths())];
...
if (import.meta.vitest != null) {
	...
	describe('loadConfig', () => {
		...
		it('should load valid configuration from .llm-water-tracker/llm-water-tracker.json', async () => {
		...
		it('should handle invalid JSON gracefully', async () => {
			await using fixture = await createFixture({
				'.llm-water-tracker/llm-water-tracker.json': '{ invalid json }',
			});
		...
		it('should prioritize local .llm-water-tracker config over Claude paths', async () => {
			await using fixture = await createFixture({
				'.llm-water-tracker/llm-water-tracker.json': JSON.stringify({
					defaults: { json: true },
		...
		it('should test configuration priority order with multiple files', async () => {
			...
			expect(config1?.source).toBe(fixture.getPath('.llm-water-tracker/llm-water-tracker.json'));
			...
			expect(config2).toBeUndefined(); // No local .llm-water-tracker and no real Claude paths
		});
		...
		it('should handle getClaudePaths() errors gracefully', async () => {
			...
			const config = loadConfig();
			expect(config).toBeDefined();
			expect(config?.source).toBe(fixture.getPath('.llm-water-tracker/llm-water-tracker.json'));
			expect(config?.defaults?.json).toBe(true);
		});

		it('should handle empty configuration file', async () => {
			await using fixture = await createFixture({
				'.llm-water-tracker/llm-water-tracker.json': '{}',
			});
		...
		it('should use validateConfigFile internally', async () => {
			...
			const validResult = validateConfigFile(fixture.getPath('.llm-water-tracker/llm-water-tracker.json'));
	...
	describe('debug functionality', () => {
		...
		describe('loadConfig with debug', () => {
			it('should log debug info when loading config with debug=true', async () => {
				...
				expect(loggerInfoSpy).toHaveBeenCalledWith(
					`  • Checking: ${fixture.getPath('.llm-water-tracker/llm-water-tracker.json')} (found ✓)`,
				);
				expect(loggerInfoSpy).toHaveBeenCalledWith('');
				expect(loggerInfoSpy).toHaveBeenCalledWith(
