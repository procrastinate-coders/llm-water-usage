 ...
 * for llm-water-tracker configuration files. The generated schema enables:
...
function createConfigSchemaJson() {
	...
	return {
		$ref: '#/definitions/tracker-config',
		definitions: {
			'tracker-config': {
				type: 'object',
				properties: {
		...
		title: 'llm-water-tracker Configuration',
...
if (import.meta.vitest != null) {
	...
	describe('createConfigSchemaJson', () => {
		it('should generate valid JSON Schema', () => {
			...
			expect(jsonSchema.$ref).toBe('#/definitions/tracker-config');
			expect(jsonSchema.definitions).toBeDefined();
			expect(jsonSchema.definitions['tracker-config']).toBeDefined();
			expect(jsonSchema.definitions['tracker-config'].type).toBe('object');
		});

		it('should include all expected properties', () => {
			const jsonSchema = createConfigSchemaJson();
			const mainSchema = jsonSchema.definitions['tracker-config'];
		...
		it('should include all command schemas', () => {
			const jsonSchema = createConfigSchemaJson();
			const commandsSchema = jsonSchema.definitions['tracker-config'].properties.commands;
