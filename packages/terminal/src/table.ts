...
export type UsageReportConfig = {
	...
	forceCompact?: boolean;
	/** Additional column headers appended after Cost (USD) (e.g. ['Water (L)', 'Water (gal)']) */
	extraHeaders?: string[];
	/** Alignments for extra columns (defaults to 'right') */
	extraAligns?: TableCellAlign[];
};
...
export type UsageData = {
	...
	totalCost: number;
	modelsUsed?: string[];
	/** Extra column values appended after cost (e.g. ['2.4500 L', '0.6472 gal']) */
	extraValues?: string[];
};
...
export function createUsageReportTable(config: UsageReportConfig): ResponsiveTable {
	...
	// Add extra columns (e.g. Water columns)
	if (config.extraHeaders != null && config.extraHeaders.length > 0) {
		for (const header of config.extraHeaders) {
			baseHeaders.push(header);
		}
		const extraAligns = config.extraAligns ?? config.extraHeaders.map(() => 'right' as TableCellAlign);
		for (const align of extraAligns) {
			baseAligns.push(align);
		}
	}
...
export function formatUsageDataRow(
	...
	if (data.extraValues != null) {
		for (const value of data.extraValues) {
			row.push(value);
		}
	}
...
export function formatTotalsRow(
	...
	if (extraTotalValues != null) {
		for (const value of extraTotalValues) {
			row.push(pc.yellow(value));
		}
	}
