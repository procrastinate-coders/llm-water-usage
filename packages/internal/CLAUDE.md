...
This package contains shared internal utilities for the llm-water-tracker monorepo.

## Package Overview

**Name**: `@llm-water-tracker/internal`
**Description**: Shared internal utilities for the llm-water-tracker toolchain
...
- Apps in this monorepo (llm-water-tracker, mcp, codex) are bundled CLIs, so all their runtime dependencies should be in `devDependencies`
...
3. Import in consuming apps as `devDependencies`:
   ...
   ```json
   "devDependencies": {
     "@llm-water-tracker/internal": "workspace:*"
   }
   ```
4. Use the utility:
   ```typescript
...
Follow the same conventions as the main llm-water-tracker package:
