	...
	async run(ctx) {
		...
		if (useJson) {
			...
			if (ctx.values.active && blocks.length === 1) {
				...
				const activeWaterMl = calculateWaterMlForEntry({ inputTokens: block.tokenCounts.inputTokens, outputTokens: block.tokenCounts.outputTokens, cacheCreationTokens: block.tokenCounts.cacheCreationInputTokens, cacheReadTokens: block.tokenCounts.cacheReadInputTokens }, block.models, waterRates);
				log(pc.bold('Water Consumption:'));
				...
				logger.box('LLM Water Tracker - Session Blocks');
