...
import { formatCurrency } from '@llm-water-tracker/terminal/table';
...
function createJsonFile<T>(filePath: string): JsonFile<T> {
...
function getSemaphore(
	sessionId: string,
): ReturnType<typeof createJsonFile<SemaphoreType | undefined>> {
	const semaphoreDir = join(tmpdir(), 'llm-water-tracker-semaphore');
	...
	const semaphore = createJsonFile<SemaphoreType>(semaphorePath);
	return semaphore;
}
...
const costSourceChoices = ['auto', 'tracker', 'cc', 'both'] as const;
...
export const statuslineCommand = define({
	...
	args: {
		...
		costSource: {
			...
			description:
				'Session cost source: auto (prefer CC then tracker), tracker (always calculate), cc (always use Claude Code cost), both (show both costs)',
			default: 'auto',
			negatable: false,
			toKebab: true,
		},
		...
		const mainProcessingResult = Result.pipe(
			await Result.try({
				try: async () => {
					...
					const { sessionCost, ccCost, trackerCost } = await (async (): Promise<{
						sessionCost?: number;
						ccCost?: number;
						trackerCost?: number;
					}> => {
						...
						// Helper function to get tracker cost
						const getTrackerCost = async (): Promise<number | undefined> => {
							return Result.pipe(
								Result.try({
									try: async () =>
						...
						if (costSource === 'both') {
							...
							const trackerCost = await getTrackerCost();
							return { ccCost, trackerCost };
						}
						...
						// If 'tracker' mode, always calculate using llm-water-tracker
						if (costSource === 'tracker') {
							const cost = await getTrackerCost();
							return { sessionCost: cost };
						}

						// If 'auto' mode (default), prefer Claude Code cost, fallback to tracker
						if (costSource === 'auto') {
							if (hookData.cost?.total_cost_usd != null) {
								return { sessionCost: hookData.cost.total_cost_usd };
							}
							// Fallback to tracker calculation
							const cost = await getTrackerCost();
							return { sessionCost: cost };
						}
					...
					const sessionDisplay = (() => {
						...
						if (ccCost != null || trackerCost != null) {
							const ccDisplay = ccCost != null ? formatCurrency(ccCost) : 'N/A';
							const trackerDisplay = trackerCost != null ? formatCurrency(trackerCost) : 'N/A';
							return `(${ccDisplay} cc / ${trackerDisplay} tracker)`;
						}
						// Single cost display
